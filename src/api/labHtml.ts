...
<style>
  ...
  *{box-sizing:border-box} body{margin:0} button,input,textarea,select{font:inherit}
  ...
  input,textarea,select{width:100%;border:1px solid #d8d3cb;border-radius:11px;padding:11px;background:#fff} textarea{min-height:430px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;line-height:1.45}
  ...
  .status{font-size:13px;margin-top:12px;color:#605b54;white-space:pre-wrap}.error{color:#9b2c2c}.success{color:#26633f}.warning{color:#855f12}
  .cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px}.card{border:1px solid #e1ddd6;border-radius:15px;padding:15px;background:#fff}.route{font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:#77716a}.card h3{font-family:Georgia,serif;font-size:24px;line-height:1.05;margin:9px 0}.meta{display:flex;gap:6px;flex-wrap:wrap;margin:10px 0}.pill{font-size:11px;background:#f2eee8;border-radius:999px;padding:5px 8px}.copy{font-size:13px;line-height:1.45;color:#4c4945}.score{font-weight:800}.renderBtn{margin-top:12px;width:100%}.preview{margin-top:12px;background:#f3f0eb;border-radius:12px;overflow:hidden;min-height:80px;display:grid;place-items:center}.preview img{width:100%;height:auto;display:block}
  .empty{border:1px dashed #cec7bd;border-radius:15px;padding:42px;text-align:center;color:#77716a}.row{display:grid;grid-template-columns:1fr 1fr;gap:10px}.mediaRow{display:grid;grid-template-columns:1fr 1fr;gap:10px}.notes{margin-top:10px;padding:10px 12px;background:#fff8e8;border-radius:10px;font-size:12px;line-height:1.4;color:#695018}.videoPlan{padding:16px;text-align:left;width:100%}.videoPlan h4{margin:0 0 8px;font-size:18px}.beat{padding:10px 0;border-top:1px solid #ddd7cf;font-size:12px;line-height:1.4}.beat:first-of-type{border-top:0}.time{font-weight:800;margin-right:6px}
  @media(max-width:900px){.grid{grid-template-columns:1fr}.top{display:block}.badge{display:inline-block;margin-top:12px}.row,.mediaRow{grid-template-columns:1fr}}
</style>
</head>
...
<div class="grid">
  <section class="panel">
    <div class="row"><div><label>Model mode</label><select id="mode"><option value="auto">AI when configured</option><option value="deterministic">Deterministic only</option></select></div><div><label>Engine token (optional)</label><input id="token" type="password" placeholder="Only if you set LUMI_ENGINE_TOKEN" /></div></div>
    <div class="mediaRow">
      <div><label>Optional founder image</label><input id="founderImage" type="file" accept="image/*"/><div class="hint">Used only for founder/photo concepts. The engine should not force it into screenshot or video formats.</div></div>
      <div><label>Optional product screenshot</label><input id="productScreenshot" type="file" accept="image/*"/><div class="hint">Use this to test annotated-screenshot and product-led SaaS concepts with real UI.</div></div>
    </div>
...
async function fileData(id){const f=$(id).files[0];if(!f)return null;return await new Promise((res,rej)=>{const reader=new FileReader();reader.onload=()=>res(reader.result);reader.onerror=rej;reader.readAsDataURL(f)})}
async function generate(){
 $('generate').disabled=true;$('status').className='status';$('status').textContent='Generating creative routes…';
 try{const input=JSON.parse($('json').value);const founder=await fileData('founderImage');const screenshot=await fileData('productScreenshot');const labAssets=[];
 if(founder){input.brand.photography.founderLed=true;labAssets.push({id:'lab-founder',type:'image',url:founder,founderPresent:true,faceVisible:true,tags:['founder','uploaded','lab'],textOverlaySuitability:78,trustPotential:92,orientation:'portrait',segments:[]})}
 if(screenshot){labAssets.push({id:'lab-screenshot',type:'screenshot',url:screenshot,founderPresent:false,faceVisible:false,tags:['product','ui','interface','dashboard','uploaded','lab'],textOverlaySuitability:72,trustPotential:78,orientation:'portrait',segments:[]})}
 if(labAssets.length) input.mediaAssets=[...(input.mediaAssets||[]),...labAssets];lastInput=input;
...
function renderCards(out){const root=$('results');root.className='cards';root.innerHTML='';out.concepts.forEach((c,i)=>{const el=document.createElement('article');el.className='card';const warn=(c.qa.warnings||[]).concat(c.mediaMatch.warnings||[]);el.innerHTML='<div class="route">'+escapeHtml(c.route.archetypeId)+'</div><h3>'+escapeHtml(c.brief.headline)+'</h3><div class="meta"><span class="pill">'+escapeHtml(c.route.format)+'</span><span class="pill">'+escapeHtml(c.route.productionTreatmentId)+'</span><span class="pill">'+escapeHtml(c.route.styleId)+'</span><span class="pill score">QA '+escapeHtml(c.qa.qualityScore)+'</span></div><div class="copy">'+escapeHtml(c.brief.supportingCopy||c.route.singleBigIdea)+'</div><div class="copy" style="margin-top:8px"><b>Media:</b> '+escapeHtml(c.mediaMatch.source)+' · '+escapeHtml(c.mediaMatch.reason)+'</div>'+(warn.length?'<div class="notes"><b>Review:</b> '+escapeHtml(warn.join(' · '))+'</div>':'')+'<button class="primary renderBtn">Render this creative</button><div class="preview" id="preview-'+i+'"><span class="hint">Not rendered yet</span></div>';el.querySelector('button').onclick=()=>renderOne(i,el.querySelector('button'));root.appendChild(el)})}
function videoPlanHtml(plan){if(!plan)return '<div class="videoPlan"><b>Video plan unavailable.</b></div>';let html='<div class="videoPlan"><h4>'+escapeHtml(plan.hook)+'</h4><div class="hint">'+escapeHtml(plan.format)+' · '+escapeHtml(plan.durationSeconds)+' seconds</div>';(plan.beats||[]).forEach(b=>{html+='<div class="beat"><span class="time">'+escapeHtml(b.start)+'–'+escapeHtml(b.end)+'s</span><b>'+escapeHtml(b.purpose)+'</b><br>'+escapeHtml(b.visual)+(b.voiceover?'<br><i>VO: '+escapeHtml(b.voiceover)+'</i>':'')+(b.onScreenText?'<br><i>On-screen: '+escapeHtml(b.onScreenText)+'</i>':'')+'</div>'});return html+'</div>'}
async function renderOne(i,btn){if(!lastInput||!lastOutput)return;const c=lastOutput.concepts[i];btn.disabled=true;btn.textContent='Rendering…';const p=$('preview-'+i);p.innerHTML='<span class="hint">Composing creative…</span>';try{const r=await fetch('/v1/creative/render',{method:'POST',headers:headers(),body:JSON.stringify({creativeId:c.route.id,input:lastInput,renderPlan:c.renderPlan})});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.message||j.error||'Render failed');const d=j.data;if(d.kind==='static'){p.innerHTML='<img alt="Rendered creative" src="'+escapeHtml(d.flattenedAsset.url)+'"/>'}else if(d.kind==='carousel'){p.innerHTML='';d.flattenedAssets.forEach(a=>{const img=document.createElement('img');img.src=a.url;p.appendChild(img)})}else if(d.kind==='video-plan'){p.innerHTML=videoPlanHtml(d.plan)}else{p.innerHTML='<span class="error">Unknown render result.</span>'}btn.textContent='Render again'}catch(e){p.innerHTML='<span class="error">'+escapeHtml(e.message||String(e))+'</span>';btn.textContent='Try render again'}finally{btn.disabled=false}}
