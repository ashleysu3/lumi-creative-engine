...
export function matchMedia(input: CreativeEngineInput, route: CreativeRoute): MediaMatch {
  ...
  if (input.brand.photography.founderLed && strongFounderExists && !best.asset.founderPresent && !route.format.startsWith('screen-recording') && route.format !== 'annotated-screenshot') {
    warnings.push('Strong founder asset exists but was not selected; review before rendering.');
  }
